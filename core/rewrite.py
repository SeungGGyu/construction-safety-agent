...
def rewrite(state: AgentState, llm):
    """
    검색 쿼리 재작성 모듈
    - llm: KANANA 또는 QWEN 객체 (main.py에서 선택적으로 주입)
    """
...
"""
    ...
    # LLM 호출 (KANANA 또는 QWEN)
    prompt = f"{system_prompt}\n\n입력:\n{q}\n출력:"
    base = _clean(llm.invoke([HumanMessage(content=prompt)]).content or q)
